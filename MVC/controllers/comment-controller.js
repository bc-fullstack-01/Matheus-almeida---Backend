...
module.exports = {
  ...
  list: (req, res, next) => Promise.resolve()
    ...
    .then((data) => {
      res.render('comments/list', {comments: data})
    })
    .catch(err => next(err)),
  add: (req, res, next) => Promise.resolve()
    ...
    .then((data) => {
      res.message('add comments success')
  ...
  show: (req, res, next) => Promise.resolve()
    ...
    .then((data) => {
      if (data) {
        res.render('comments/show', {comment: data})
      } else {
  ...
  save : (req, res, next) => Promise.resolve()
    .then(() => Comment.findByIdAndUpdate(req.params.id, req.body.comment, {runValidators: true}))
    .then((data) => {
  ...
  edit: (req, res, next) => Promise.resolve()
    .then(() => Comment.findById(req.params.id))
    .then((data) => {
      res.render('comments/edit', {comment: data})
    })
    .catch(err => next(err)),
